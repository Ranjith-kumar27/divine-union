...
class RegistrationBloc extends Bloc<RegistrationEvent, RegistrationState> {
  final DummyAuthRepository repo;
  ...
  Future<void> _onMobileSubmitted(
    ...
    emit(
      state.copyWith(status: RegistrationStatus.loading, mobile: event.mobile),
    );
    try {
      final otp = await repo.sendOtp(event.mobile);
      emit(state.copyWith(status: RegistrationStatus.otpSent, otpCode: otp));
    } catch (e) {
      emit(
  ...
  Future<void> _onOtpSubmitted(
    ...
    try {
      final ok = await repo.verifyOtp(event.mobile, event.otp);
      if (ok)
        emit(state.copyWith(status: RegistrationStatus.verified));
      else
        emit(
          state.copyWith(
  ...
  void _onProfileTypeSelected(
    ...
    emit(state.copyWith(profileType: event.type));
  }
  ...
  void _onPersonalDetailsUpdated(
    ...
    emit(state.copyWith(personalDetails: merged));
  }
  ...
  void _onReligionAnswerUpdated(
    ...
    emit(state.copyWith(religionAnswers: merged));
  }
